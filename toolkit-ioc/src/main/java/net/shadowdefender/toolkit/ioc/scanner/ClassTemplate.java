...
import net.shadowdefender.toolkit.lang.ClassUtils;
...
abstract class ClassTemplate {
    ...
    private void doAddClass(List<Class<?>> classList, String className) {
        // 加载类
        Class<?> calzz = ClassUtils.loadClass(className, false);
        // 判断是否可以添加类
        if (canAddClass(calzz)) {
            // 添加类
            classList.add(calzz);
        }
    }
     ...
     * @return
     */
    public abstract boolean canAddClass(Class<?> clazz);
}